...
def process_data(diff, h):
    ...
    ens_m_deviation = np.zeros([2000, 2, len(fnames_benchmark)])
    ...
    for i in range(len(fnames_comparison)):
        ...
        e_mean = tmp_comparison['e_mean']
        r_mean = tmp_comparison['r_mean']
        t_mean = tmp_benchmark['t_mean']

        ens_m_deviation[:, 0, i] = rms_deviation(t_mean, e_mean)
        ens_m_deviation[:, 1, i] = rms_deviation(t_mean, r_mean)

    
    e_med, e_h, e_min, e_max = five_num_summary(np.squeeze(ens_m_deviation[:,0,:]), 1)
    r_med, r_h, r_min, r_max = five_num_summary(np.squeeze(ens_m_deviation[:,1,:]), 1)
...
f = open('./ens_sum_stats/ens_mean_' + str(h).zfill(3) + '.txt', 'wb')
pickle.dump(data, f)
f.close()
