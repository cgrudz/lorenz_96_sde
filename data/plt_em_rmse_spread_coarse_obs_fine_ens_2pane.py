...
fnames = sorted(glob.glob('./ens_bias/fine_ensemble_step/coarse_obs_step/ens_bias_diff*'))
...
spread = np.round(spread / rk_fine_spread, decimals=3)
...
#color_spread = sns.cubehelix_palette(n_colors=80, start=1.90, light=.95)
...
rmse_bnd = np.max(np.abs(rmse))

sns.heatmap(rmse, linewidth=0.5, ax=ax0, cbar_ax=ax3, vmax=0.0075, vmin=-0.0075, cmap=color_rmse, norm=norm_1)
sns.heatmap(spread, linewidth=0.5, ax=ax1, cbar_ax=ax2, vmax=1.03, vmin=1.0, cmap=color_spread, norm=norm_2)
