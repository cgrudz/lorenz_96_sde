...
fnames = sorted(glob.glob('./ens_bias/coarse_ensemble_step/fine_obs_step/ens_bias_diff*'))
...
color_spread = sns.diverging_palette(220, 20, n=80)
...
spread_bnd = np.abs(1 - np.max(spread))


sns.heatmap(rmse, linewidth=0.5, ax=ax0, cbar_ax=ax3, vmax=.0075, vmin=-.0075, cmap=color_rmse, norm=norm_1)
sns.heatmap(spread, linewidth=0.5, ax=ax1, cbar_ax=ax2, vmax=1.002, vmin=.998, cmap=color_spread,norm=norm_2)
