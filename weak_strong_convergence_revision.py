...
def experiment(args):
    ...
    for N in range(N_ens):
        ...
        for i in range(len(dt)):
            ...
            for k in range(nanl):
                ...
                # compute the fourier coefficients of the brownian bridge,
                # where the standard normals are scaled to appropriate variance
                # fine_Delta
                a, b = compute_a_b(W[:, k * t_w: (k + 1) * t_w] * \
                        np.sqrt(fine_Delta), fine_Delta, h, p)
    ...
    fname = './data/weak_strong_convergence_delta_' + str(Delta).zfill(3) + \
            '_gamma_' + str(gamma) + '_beta_' + str(beta).zfill(2) + '_tanl_' + \
            str(T) + '_nanl_' + str(N_ens).zfill(4) + '_diff_' + str(diff).zfill(2) + \
...
f = open('./data/obs_trajs/tay_obs_seed_000_sys_dim_10_analint_001_diffusion_0.5.txt', 'rb')
tmp = pickle.load(f)
f.close()
...
## code below used for batch submission through slurm
#
#j = int(sys.argv[1])
#f = open('./data/weak_strong_revision_input','rb')
#data = pickle.load(f)
#args = data[j]
#f.close()
#
